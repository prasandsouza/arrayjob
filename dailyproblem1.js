...
//finding all web developer
function webdeveloper(data){
    let result = data.reduce((accumulator,currentValue)=>{
        if(currentValue.job==="Web Developer III" || currentValue.job==="Web Developer II" || currentValue.job==="Web Developer III" ){
...
function salary(data){
    return salaryResult = data.reduce((numberSalary,stringSalary)=>{
    let number = stringSalary.salary.slice(1)
    stringSalary.salary = parseFloat(number)
    numberSalary.push(stringSalary)
        return numberSalary
    },[])
}
...
//factor the salary
function factorsalary(salarydata){
    return factors = salarydata.reduce((accumulator,currentValue)=>{
        currentValue.newsalary = currentValue.salary * 1000
        accumulator.push(currentValue)
        return accumulator;
    },[])
}
...
console.log(countrysalary(numberOfSalary))
...
console.log(averagesalary(numberOfSalary))